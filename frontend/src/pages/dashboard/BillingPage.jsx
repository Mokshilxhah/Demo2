...
export default function BillingPage() {
  ...
  const [couponCode, setCouponCode] = useState(() => {
    return localStorage.getItem('dq-pending-coupon') || '';
  });
  ...
  useEffect(() => {
    const autoApplyCoupon = async () => {
      const pendingCoupon = localStorage.getItem('dq-pending-coupon');
  ...
  const handleVerifyPayment = async (payload) => {
    try {
      ...
      const data = await res.json();
      if (data.success) {
        toast.success(payload.mode === 'simulator' ? 'Simulated Payment Successful!' : 'Payment Verified Successfully!');
  ...
  // Handle Mock checkout action
  const handleSimulatorCheckout = (success) => {
    if (!success) {
      toast.error('Simulated Payment Failed / Rejected');
      setShowSimulator(false);
      setPayingCard(null);
      setSimData(null);
      return;
    ...
    // Trigger mock validation verification
    handleVerifyPayment({
      projectId: simData.projectId,
      razorpayOrderId: simData.orderId,
      razorpayPaymentId: `pay_mock_${Date.now()}`,
      mode: 'simulator',
      couponCode: simData.couponCode
    });
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '28px' }}>
      ...
      {/* 
        ====================================================
        SIMULATOR MODE INTERACTIVE MODAL (HIGH FIDELITY DIALOG)
        ====================================================
      */}
      {showSimulator && simData && (
        <div style={{
          ...
          <div className="dash-card" style={{
            ...
            padding: '28px',
            animation: 'modal-bounce 0.3s cubic-bezier(0.175, 0.885, 0.32, 1.275) forwards',
            color: 'var(--dash-text)'
          }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', color: 'var(--dash-accent)', borderBottom: '1px solid var(--dash-border)', paddingBottom: '14px', marginBottom: '20px' }}>
              <Sparkles size={22} />
              <h3 style={{ fontFamily: 'var(--dash-font-display)', fontSize: '1.25rem', fontWeight: 700, margin: 0 }}>Razorpay Simulator Gate</h3>
            </div>

            <p style={{ fontSize: '0.88rem', color: 'var(--dash-text-sec)', lineHeight: 1.5, margin: '0 0 16px' }}>
              Your server keys are set to simulator mode. You can test payment capture, status triggers, and verification updates without any credentials.
            </p>

            <div style={{ background: 'var(--dash-bg)', padding: '16px', borderRadius: '8px', marginBottom: '24px', fontSize: '0.85rem', display: 'flex', flexDirection: 'column', gap: '8px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <span style={{ color: 'var(--dash-text-sec)' }}>Project Name:</span>
                <strong style={{ whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: '200px' }}>{selectedProject.projectName}</strong>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <span style={{ color: 'var(--dash-text-sec)' }}>Simulated Bill:</span>
                <strong style={{ color: 'var(--dash-accent)', fontSize: '1rem' }}>₹{(simData.amount / 100).toLocaleString('en-IN')}</strong>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <span style={{ color: 'var(--dash-text-sec)' }}>Mock Order ID:</span>
                <code style={{ fontSize: '0.75rem', fontFamily: 'monospace' }}>{simData.orderId}</code>
              </div>
              {simData.couponCode && (
                <div style={{ display: 'flex', justifyContent: 'space-between', color: 'var(--dash-success)' }}>
                  <span>Coupon discount applied:</span>
                  <strong>{simData.couponCode}</strong>
                </div>
              )}
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              <button 
                onClick={() => handleSimulatorCheckout(true)}
                className="dash-btn dash-btn-primary"
                style={{ width: '100%', background: 'var(--dash-success)', border: 'none', color: '#FFF' }}
              >
                Simulate Successful Payment ✓
              </button>
              <button 
                onClick={() => handleSimulatorCheckout(false)}
                className="dash-btn"
                style={{ width: '100%', background: 'rgba(220,38,38,0.1)', color: 'var(--dash-danger)', border: '1px solid var(--dash-danger)' }}
              >
