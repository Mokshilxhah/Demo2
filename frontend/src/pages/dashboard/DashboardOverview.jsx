...
export default function DashboardOverview() {
  ...
  const getStageBadge = (stage) => {
    switch (stage) {
      ...
      default:
        return <span className="dash-badge dash-badge-warning">{stage}</span>;
    }
  };
                ...
                {data.recentProjects.map((p) => (
                  <tr key={p._id}>
                    ...
                    <td style={{ fontFamily: 'var(--dash-font-mono)', fontWeight: 600 }}>
                      {p.priceEstimated ? `₹${p.estimatedPrice.toLocaleString()}` : <span style={{ color: 'var(--dash-text-muted)' }}>Pending Quote</span>}
                    </td>
                    <td>
